...
async function checkUnpaid() {
    ...
    if (o.status !== 'ຍົກເລີກອໍເດີ') {
      ...
      if (ymOf(dateStr) === '2026-06') {
        ...
        if (pm.includes('COD') || pm.includes('ປາຍທາງ')) {
          // If status is not completed/shipped, it's unpaid COD.
          // Let's check status:
          if (o.status !== 'ส่งสำเร็จแล้ว' && o.Status !== 'ສໍາເລັດ') {
  ...
  console.log('Unpaid COD for June 2026:', unpaidCOD);
  process.exit(0);
}
